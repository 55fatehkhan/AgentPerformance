...
const getCampaignIds = (center, provider, campaign) => {
  if (center === "Shark") {
    if (provider === "telcast") {
      if (campaign === "Outbound")
        return [
          "SH_HomeWarrantyGold",
          "SH_HomeWarrantyGold ",
          "SH_HomeWarrantyGold2",
          "SH_HomeWarrantyGold2 ",
          "Shark_HomeWarrantyGold ",
        ];
      if (campaign === "Inbound")
        return ["Inbound Campaign", "Inbound Inbound Campaign "];
      if (campaign === "Both")
        return [
          "SH_HomeWarrantyGold",
          "SH_HomeWarrantyGold ",
    ...
    if (provider === "telcast") {
      ...
      if (campaign === "Inbound")
        return ["FT_Inbound_CB ", "Inbound Campaign", "Inbound Campaign "];
      if (campaign === "Both")
        return [
          "FORTUNE_JARED",
          "FORTUNE_JARED ",
...
require("./cron/postToClientJobLatest")(InboundLeads, Realtimelead);
...
app.post("/api/RetellCallLogs", async (req, res) => {
  ...
  let matchConditions1 = {
    ...
    "retellCallAnalysedLogs.direction": {
      $in: ensureArray(direction) || ["inbound", "outbound"],
    },
  };
  ...
  if (call_disposition && !call_disposition.includes("All")) {
    matchConditions2[
      "retellCallAnalysedLogs.call_analysis.custom_analysis_data.call_disposition"
    ] = { $in: ensureArray(call_disposition) };
  }

  if (disconnect_reason) {
    matchConditions2["retellCallAnalysedLogs.disconnection_reason"] = {
      $in: ensureArray(disconnect_reason),
    };
  }
  ...
  const pipeline = [
    ...
    { $match: matchConditions1 },
    {
      $addFields: {
        duration_second: {
          $divide: ["$retellCallAnalysedLogs.duration_ms", 1000],
        },
      },
    },
    { $match: matchConditions2 },
    {
      $project: {
        ...
        recording_url: {
          $ifNull: ["$retellCallAnalysedLogs.recording_url", ""],
        },
        disconnect_reason: {
          $ifNull: ["$retellCallAnalysedLogs.disconnection_reason", ""],
        },
        call_cost: {
          $divide: ["$retellCallAnalysedLogs.call_cost.combined_cost", 100],
        },
        call_disposition:
          "$retellCallAnalysedLogs.call_analysis.custom_analysis_data.call_disposition",
  ...
  const pipeline = [
    ...
    },
    {
      $group: {
        ...
        Human: {
          $sum: {
            $cond: [{ $eq: ["$classification", "HUMAN"] }, 1, 0],
          },
        },
        Machine: {
          $sum: {
            $cond: [{ $eq: ["$classification", "MACHINE"] }, 1, 0],
          },
        },
        GrandTotal: { $sum: 1 },
      },
...
app.post("/api/RetellAttemptCount", async (req, res) => {
  ...
  if (disconnect_reason) {
    matchConditions["retellCallAnalysedLogs.disconnection_reason"] = {
      $in: ensureArray(disconnect_reason),
    };
  }
  // console.log("Matched2:  ", matchConditions2);
    ...
    },
    {
      $project: {
        ...
        retellCallAnalysedLogs: {
          $arrayElemAt: [{ $reverseArray: "$retellCallAnalysedLogs" }, 0],
        },
      },
    },
    {
  ...
  const pipeline = [
    ...
    },
    {
      $project: {
          ...
          $round: [
            {
              $ifNull: [
                {
                  $divide: ["$body.call.duration_ms", 1000],
                },
                0,
              ],
            },
        ...
        disconnect_reason: {
          $ifNull: ["$body.call.disconnection_reason", ""],
        },
        retell_call_cost: {
          $divide: ["$body.call.call_cost.combined_cost", 100],
        },
        call_disposition:
        ...
        createdAtSimple: 1,
        twilioCallDuration: "$twilio.twilioRecordingDuration",
        twilioCallDurationWithRing: "$twilio.twilioCallDuration",
        // dial_index: 1,
        // subId: 1,
        // source: 1,
        // campaign: 1,
...
app.post("/api/AgentSaleReport", async (req, res) => {
  try {
    ...
    if (campaignIds.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid selection for campaign filtering" });
    }
...
app.post("/post-to-client", async (req, res) => {
  try {
    ...
    let url = `https://choicehomewarranty.com/host-post.php?NETWORKID=sclPT&AFID=blindxfer&FirstName=${encodeURIComponent(
      firstName
    )}&LastName=${encodeURIComponent(lastName)}&Address=${encodeURIComponent(
      address
    )}&City=${encodeURIComponent(city)}&State=${encodeURIComponent(
      state
    )}&PostalCode=${encodeURIComponent(postalCode)}&Phone=${encodeURIComponent(
      phone
    )}&Email=${encodeURIComponent(
      email
    )}&IPAddress=208.109.184.203&_OwnHome=Yes&_optin=Yes&Team=b`;
    ...
    if (response.ok) {
      ...
      if (lead) {
        console.log(
          `Successfully updated postToClient to true for phone: ${phone}`
        );
      } else {
...
app.post("/api/clientReport", async (req, res) => {
  try {
    ...
    // Handle `centerName`
    if (centerName) {
      matchConditions["Center"] = Array.isArray(centerName)
        ? { $in: centerName }
        : centerName;
    }
    ...
    // Handle `provider`
    if (provider) {
      matchConditions["campaign2"] = Array.isArray(provider)
        ? { $in: provider }
        : provider;
    }
    ...
    // Handle `BiweeklyDate`
    if (BiweeklyDate) {
      matchConditions["Date"] = Array.isArray(BiweeklyDate)
        ? { $in: BiweeklyDate }
        : BiweeklyDate;
    }
...
app.post("/api/voipcosts/filters", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching filtered VoIP costs", error });
  }
});
    ...
    const aggregationPipeline = [
      ...
      // Step 4: Convert total duration to minutes
      {
        $addFields: {
          totalDurationMinutes: {
            $round: [{ $divide: ["$totalDurationSeconds", 60] }, 2],
          },
        },
      },
      ...
      // Step 6: Add Total Attempts, Total Paid, and calculations
      {
        $addFields: {
          ...
          "Total Calls per Attempts": {
            $cond: {
              if: {
                $gt: [{ $arrayElemAt: ["$attemptsData.totalAttempts", 0] }, 0],
              },
              then: {
                $round: [
                  {
                    $divide: [
                      "$Human",
                      { $arrayElemAt: ["$attemptsData.totalAttempts", 0] },
                    ],
                  },
                  2,
                ],
          ...
          "No of Calls Per Paid": {
            $cond: {
              if: {
                $gt: [{ $arrayElemAt: ["$attemptsData.totalPaid", 0] }, 0],
              },
              then: {
                $round: [
                  {
        ...
        $addFields: {
          "agentsData.HumanAnswerPercentageOutofTotalHA": {
            $round: [
              {
                $multiply: [
                  { $divide: ["$agentsData.Human", "$totalHumanAnswers"] },
                  100,
                ],
              },
              2,
      ...
      // Step 9: Lookup and allocate cost from voipcosts
      {
        $lookup: {
          from: "voipcosts",
          let: {
            center: centerNameFilter,
            provider: dialerFilter,
        ...
        $addFields: {
          ...
          allocatedCost: {
            $round: [
              {
                $multiply: [
                  "$HumanAnswerPercentageOutofTotalHA",
                  { $divide: ["$totalVoipCost", 100] },
                ],
              },
              2,
            ],
...
app.post("/api/filePerformance", async (req, res) => {
  ...
  if (listId && listId.length) {
    matchCondition.Source_listId = {
      $in: Array.isArray(listId) ? listId : [listId],
    };
  }

  // Add dataset filter only if it is valid
  if (dataset !== null && dataset !== undefined && !isNaN(dataset)) {
    matchCondition.dataset = {
      $in: Array.isArray(dataset) ? dataset : [dataset],
    };
  }
